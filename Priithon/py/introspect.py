...
__cvsid__ = "$Id: introspect.py,v 1.11.2.2 2006/06/29 22:24:00 RD Exp $"
__revision__ = "$Revision: 1.11.2.2 $"[11:-2]
...
try:
    ...
    def fnst():
        ...
        for tt in N.ScalarType:
            ...
            # l is list of 'N' members (names) for type tt
            #remove all names with '_'
            checkIfSomeNamesShouldGoAway = len(l) > 1
            while checkIfSomeNamesShouldGoAway:
                for i in range(len(l)):
                    ...
                    checkIfSomeNamesShouldGoAway = 0

            #remove all names that do NOT contains a digit
            checkIfSomeNamesShouldGoAway = len(l) > 1
            while checkIfSomeNamesShouldGoAway:
                for i in range(len(l)):
                    if not re.findall('[0-9]+', l[i]):
                        del l[i]
                        checkIfSomeNamesShouldGoAway = len(l) > 1
                        break
                else:
...
import __builtin__
_builtin_inverse_lookup_table=dict([(v,k) for k,v in __builtin__.__dict__.iteritems()
                                    if type(v) not in (bool, type(None), str, unicode, int, float) ]) # CHECK
...
def getCallTip(command='', locals=None):
    ...
    if inspect.isbuiltin(object):
        ...
        #seb
        # seb's COMPLAIN: default float args show like f(a=0.59999999999999998)
        # seb's COMPLAIN:      instead of              f(a=0.6)
        ...
        #          def fixFloat(x):
        #              import __builtin__
        #              if type(x) == __builtin__.float:
        #                  return str(x)
        #              else:
        #                  return x
        #          defaults = map(fixFloat, defaults)
        #defaults = map(str, defaults)
        #argspec = (args, varargs, varkw, defaults)
        #print "seb2a:::",  argspec
        #argspec = apply(inspect.formatargspec, argspec)

        #  formatargspec(args, varargs=None, varkw=None, defaults=None,
        #                formatarg=<type 'str'>,
        #                formatvarargs=<function <lambda>>,
        #                formatvarkw=<function <lambda>>,
        #                formatvalue=<function <lambda>>,
        #                join=<function joinseq>)
        ...
        #DEBUG    (args, varargs, varkw, defaults)=inspect.getargspec(object)
        #DEBUG    argspec = (args, varargs, varkw, defaults)
        #DEBUG    print "seb2a:::",  argspec

        def sebValForm(v):
            #import __builtin__
            #if type(v) == __builtin__.float:
            ...
            try:
                s=_builtin_inverse_lookup_table[v]
                return '='+s
            except: # KeyError:  -- also: e.g. TypeError: list objects are unhashable
                pass

            if type(v) == str:
                return '="' + str(v) + '"'  # keep "aa" instead of aa
            elif type(v) == unicode:
                return '=u"' + unicode(v) + '"'  # keep u"aa" instead of aa
            #elif shouldAddNA(v): 
            #    return '=na.' + str(v)      # show type=na.Float  instead of  type=Float
            elif type(v) is type and v in numpyScalarTypes.keys():
                # show dype=N.Float  instead of  dtype=<type 'float32scalar'>
                return '=N.' + numpyScalarTypes[v]

            # elif shouldAddN(v): 
            #     return '=N.' + str(v)      # show dype=N.Float  instead of  dtype=Float
            else:
                return '=' + str(v)         # show .6 instead of  .59999999
            #else:
            #    return '=' + repr(v)
            
        #orig        argspec = apply(inspect.formatargspec, inspect.getargspec(object))
        argspec = apply(inspect.formatargspec, inspect.getargspec(object),
                        {'formatvalue': sebValForm})
